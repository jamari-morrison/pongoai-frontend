...
import { useId, useControllableState, useBoolean, useEventCallback, useMergedRefs, clamp, } from '@fluentui/react-utilities';
...
export const useSelectState = (state) => {
    ...
    const createOptions = React.useMemo(() => {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            items.push(React.createElement("li", { className: options[i].value === options[currentOptionIndex].value
                    ...
                    : listClassName + ' inactive', ref: optionItemsRef.current[i], role: "option", "aria-selected": options[i].value === options[currentOptionIndex].value, "data-value": value, onClick: onOptionClick, onKeyUp: onOptionKeyUp, onKeyDown: onOptionKeyDown, key: i, tabIndex: -1 }, label));
        }
        return items;
    ...
    prefix
        ...
        : (state.textFieldPrefix.children = React.createElement(Chevron, { visible: visible, disabled: disabled }));
    // Suffix Props
