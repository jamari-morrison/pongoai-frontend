...
var useButtonStyles = function (state) {
    ...
    state.root.className = react_make_styles_1.mergeClasses(rootStyles.root, state.disabled ? rootStyles[state.color + 'Disabled'] : rootStyles[state.color], !state.disabled && rootStyles.focusIndicator, state.disabled ? rootStyles.disabled : rootStyles.enabled, rootStyles[state.appearance], rootStyles[state.size], rootStyles[state.shape], state.appearance === 'transparent' && !state.disabled && rootStyles.transparentEnabled, state.root.className);
    return state;
};
