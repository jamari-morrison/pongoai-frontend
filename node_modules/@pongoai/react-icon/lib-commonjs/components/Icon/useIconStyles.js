"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.useIconStyles = exports.useRootStyles = void 0;
const react_make_styles_1 = require("@fluentui/react-make-styles");
exports.useRootStyles = react_make_styles_1.makeStyles({
    smaller: {
        width: '20px',
        height: '20px',
        '& svg': {
            width: '20px',
            height: '20px',
        },
    },
    small: {
        width: '24px',
        height: '24px',
        '& svg': {
            width: '24px',
            height: '24px',
        },
    },
    medium: {
        width: '28px',
        height: '28px',
        '& svg': {
            width: '28px',
            height: '28px',
        },
    },
    large: {
        width: '32px',
        height: '32px',
        '& svg': {
            width: '32px',
            height: '32px',
        },
    },
    inherit: (theme) => ({
        '& svg': {
            fill: theme.palette.inherit,
        },
    }),
    neutral: (theme) => ({
        '& svg': {
            fill: theme.palette.neutral,
        },
    }),
    brand: (theme) => ({
        '& svg': {
            fill: theme.palette.brand,
        },
    }),
    secondary: (theme) => ({
        '& svg': {
            fill: theme.palette.secondary,
        },
    }),
    success: (theme) => ({
        '& svg': {
            fill: theme.palette.success,
        },
    }),
    error: (theme) => ({
        '& svg': {
            fill: theme.palette.error,
        },
    }),
    social: (theme) => ({
        '& svg': {
            fill: theme.palette.social,
        },
    }),
    warning: (theme) => ({
        '& svg': {
            fill: theme.palette.warning,
        },
    }),
    info: (theme) => ({
        '& svg': {
            fill: theme.palette.info,
        },
    }),
});
const useIconStyles = (state) => {
    const rootStyles = exports.useRootStyles();
    state.root.className = react_make_styles_1.mergeClasses(rootStyles[state.color], rootStyles[state.size], state.root.className);
    return state;
};
exports.useIconStyles = useIconStyles;
//# sourceMappingURL=useIconStyles.js.map